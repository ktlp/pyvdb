...
* C++ source files:
  * `include/pyvdb/pyvdb.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/pyvdb.cpp` is the main file that implements this library.
  * `app/pyvdb_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/pyvdb_t.cpp` contains the unit tests for the library.
  ...
  * The `python/pyvdb` directory contains a Python
    package for the project. It contains a compiled Python module `_pyvdb`
    that CMake generates from the Pybind11 source file `python/pyvdb/_pyvdb.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
...
* CMake build system files
  ...
  * `pyvdbConfig.cmake.in` provides a template for the configuration
    installed alongside your project. This is required to implement the transitivity of your dependency
    on `openvdb`: If downstream projects use your library, they should
    automatically search for `openvdb`. The config file template implements
    exactly this logic.
